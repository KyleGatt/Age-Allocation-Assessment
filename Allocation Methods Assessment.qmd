...
Since 2017, the age allocation method has achieved an average CV score of 28.5 across all fisheries, stocks, and years. Precision was highest for the Susitna (CV = 22.3) and Other (CV = 23.31) stocks but lowest for the Crescent (CV=45.2), Kasilof (CV = 29.3) and Fish Creek (CV = 29.5) stocks. The Central District Drift (CV = 19.1) and Westside fisheries (CV = 24.3) achieved the highest average precision. The age allocation method has had an average absolute error in estimating the commercial harvest of Kenai and Kasilof stocks of 41,900 and 35,800 fish respectively.
...
The GSI allocation method projected a much larger total run than did the age allocation model and would resulted in a mid-season tier designation change in 2024. This result was somewhat anticipated, as the GSI method presented here assumes a static stock composition within each fishery. This static approach tends to overestimate Kenai contributions early in the season, before the stock’s peak run timing, resulting in inflated total run projection. The GSI allocation method presented here is a broad-stroke approach based on aggregated annual GSI estimates and does not account for temporal variation in stock-compositions. If we incorporate some temporal variation into the GSI allocation model by assuming a 50% reduction in the Kenai contribution prior to July 20th across all fisheries, the GSI allocation method projects a similar total run estimate as the age allocation model and is near the true estimate (@fig-Temporalcurve, @fig-Temporalselect, @tbl-Temporaltable) . This correction is based on 2023 GSI data from the Central District Drift harvest that showed the Kenia contribution to corridor-only harvests increase on average from 38% prior to July 20th to over 72% after July 20th.

**Overall, adoption of the GSI allocation approach presents several advantages: it yields similar final total run estimates as the age allocation model, it is far more simpler to employ, and reduces the need for timely in-season age composition data which will allow for re-allocation of resources to other sampling efforts (e.g., Northern District North, Coho sampling). However, refinements are needed before full implementation. Next step for this transition to occur include:**

-   **Staff need to seek biometric support to refine modeling techniques.**

-   **Develop a GSI database using raw non-aggregated GSI data.**

-   **Refine GSI allocations to apportion by statistical areas rather than aggregated broad areas (this will account for differences in corridor and non-corridor only periods and variable run timing to districts).**

-   **Design catch and escapement sampling goals to maximize GSI temporal resolution.**
